...
- `build-pools` 输入是 cleaned dialogs JSONL + domain；`clean-dialogs` 是可选前置工具。处理 JSONL 前必须先读一行确认 schema。
- `build-pools` 输出四类文件：
...
- `noise_event_pool_candidate` 是三层噪声候选池，包含 `surface_noise`、`semantic_noise` 和 `pragmatic_noise`，不是 gold label。默认 `surface_noise` 由本地规则抽取，`semantic_noise` / `pragmatic_noise` 由 Chatdemo 抽取；可用 `--noise-event-mode rules|off` 切换。
- `noise_event_pool_review.md` 必须标明哪些 type 最需要人工质检；semantic/pragmatic 噪声以及 `asr_garbled`、`code_mixing`、`truncated_utterance` 默认需要 review 后再用于正式 benchmark。
- `behavior_pool` 额外保留 `why_useful`。
- 任何生成/抽取阶段出现少量失败时，默认先做定点修复：读取 `stage1_failed.jsonl`、`noise_event_failed.jsonl`、`profile_spec_failed.json` 或 `failed_profiles.jsonl`，只补失败 ID，然后合并并重建下游依赖文件。不要因为 1 条或少量失败直接全量重跑；只有全局配置、模板、脚本或依赖错误影响所有记录时才全量重跑。
- `build_pools.py --only-stage2` 用于只从已有 `stage1_per_conversation.jsonl` 修复 `stage2_typical_personas.jsonl`，不重跑 noise/event/pool。`--skip-stage1` 只适合复用 stage1 后重建后续 pools，不能当作“只修 stage2”的入口。
...
- difficulty 不按 `user_starting_position`、`convertibility_ceiling`、最终结局或 persona 直接映射；它只描述用户对抗行为的持续性。拒绝还款可以是业务失败，但不必然是 L2/L3。
- `convertibility_ceiling` 从业务结果角度定义。对 CL 催收来说，用户拒绝还款属于 Failure / Low-No Convertibility，因为催收没有成功。
...
- `gen.md` 必须自动生成，但需要 human confirm 后再用于 profile generation。`gen.md` 是路由型薄入口，只保留 source map、canonical decisions 和 profile 生成契约；不要把所有中间分析全文拼接进去。
- `derive-guide` 必须生成 `baseline_user_model.md`，并把它写入 `gen.md` 固定章节；该章节描述真实外呼用户的默认注意力、耐心、防备心理、表达质量、打断/挂断倾向、domain-specific adaptation 和 non-transferable assumptions，避免 profile 过度配合、过度清晰或像测试用户。不要生成固定的 engagement state transitions 章节。
...
| 文件 | 路径 | 用途 |
|------|------|------|
| 当前 TM smoke 输入 | `benchmark_1_test/demo/tm_pipeline_smoke/` | cleaned dialogs、prompts、pools、guide 的试跑目录 |
| TM prompts 示例 | `benchmark_1_test/demo/tm_pipeline_smoke/marketing_prompts_v2.jsonl` | 37 条营销 prompt |
| 自动生成规范 | `benchmark_1_test/demo/tm_pipeline_smoke/guide/gen.md` | derive-guide 生成的薄入口规范，人工确认后使用 |
| 旧版 Profile 生成规范 | `benchmark_1_test/gen_v2.md` | 历史参考，不能覆盖新链路产物 |
...
| 文件 | 路径 | 用途 |
|------|------|------|
| 当前 CL smoke 目录 | `benchmark_1_test/demo/cl_pipeline_smoke/` | CL 已整理的 inputs / pools / guide / dialogs |
...
1. **设计文档**：用哪个设计表？
   - TM：优先使用当前自动链路生成的 `profile_spec.json`；旧 `demo_profile_design_1/2` 仅作 legacy 参考。
   - CL：优先使用 `demo/cl_pipeline_smoke/inputs/m1_m3_profile_spec_24.json` 或当前链路生成的 spec；旧 `cl/m1_m3_profile_design.md` 仅作 legacy 参考。

2. **Prompts 文件**：
   - TM 默认：`benchmark_1_test/demo/tm_pipeline_smoke/marketing_prompts_v2.jsonl`；如不存在则询问用户。
   - CL 默认：`benchmark_1_test/demo/cl_pipeline_smoke/inputs/m1_m3_prompts_from_md_no_prompt_card.jsonl`
...
1. **Profile 文件**：用哪个 profile JSONL？
   - TM 默认：当前 `generate-profiles` 输出的 `profiles_runtime.jsonl`
   - CL 默认：当前 `generate-profiles` 输出的 `profiles_runtime.jsonl`
